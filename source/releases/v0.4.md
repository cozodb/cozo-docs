...
Eventually, your social network took over the world, with the simple schema in CozoScript:

```
:create status {uid: String, ts: Validity default 'ASSERT' => mood: String}
```

and the query you ran to generate the home page is:
...
Obviously, we can no longer provide SQL translations.
